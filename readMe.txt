...
2023-01-11
1.调整强声器使能禁止的逻辑，改为控制继电器


2023-01-12
1.api初始化默认手柄mic输入，音量调整（0x1a,0x1b），内部回环取消（0x26）
2.手柄ptt控制morse输出（morse开关自动输出电压）


2023-01-14
1.取消8388自动增益（0x12修改为0x3a）
2.修改io检测逻辑，防止获取按键线程异常。
2.drv722_22134_server 默认关闭log，可以在运行时指定参数-L 表示开启日志
...
2023-03-07
1.扬声器音量调节的时候，音量50%的时候声音太小，进行了一个微调drv722_22134_api.c  drvSetSpeakVolume函数



2023-03-16
1.在server中也增加了对8388的初始化的语句，尤其是寄存器3的值，设置为08，这样mic就有声音了。
2.es8388_find_iic_devaddr这个函数不再使用，因为调整了8388的驱动，用这个函数无法查找到8388，所以就导致打开失败。
