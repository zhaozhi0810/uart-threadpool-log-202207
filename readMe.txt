...
2022-09-28
1.串口通信，在接收时的等待时间太长，改为100us的循环等待。这能加速串口的响应。
如果等待是100ms，结果响应只要1ms，那就白等了99ms，反而减慢了串口数据的发送，因为至少等待100ms才能进行下一次串口通信。
2.去除之前使用的drv722_22134_server 后台运行的服务程序，不再使用该程序。使用更简单方便
3.增加一个led键灯闪烁控制的接口drvFlashLEDs(int nKeyIndex)。 对应的头文件（drv_22134_api.h）已经更新
4.如果调用闪烁接口之后，需要调用drvLightLED(int nKeyIndex)或者drvDimLED(int nKeyIndex)表示停止闪烁后led停留的状态。
5.因客户需要server程序，本分支程序不再更新。
