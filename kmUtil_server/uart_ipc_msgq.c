...
// #define KEY_ID -123123




static int uart_msgid = -1;  //消息队列的描述符


static int uart_MsgClear(void)
{
	uart_msgq_t msgbuf;
	...
	while( msgrcv(uart_msgid, &msgbuf, sizeof(msgbuf)-sizeof(long),0,IPC_NOWAIT) >0) ;  //关闭的时候把所有的消息都清除掉
...
//	int retry = 0;
    ...
    //接收消息
    do{
	    ret = msgrcv(uart_msgid, msgbuf, sizeof(uart_msgq_t)-sizeof(long),types,recv_flg);  //防止死循环
	    if(ret >= 0)
	    	break;
...
//	int cmd = msgbuf->cmd;
    ...
    //发送消息
    if(msgsnd(uart_msgid, msgbuf, sizeof(uart_msgq_t)-sizeof(long),0) == 0)
    {
    //	printf("debug:msgsnd ok \n");
    	//等待应答，等待的类型跟命令有关！！！！！
    	ret =  Jc_uart_msgq_recv(ack_types,msgbuf,timeout);
